...
ps_module ps_mod_dynomite = {
    PS_MOD(dynomite)
};

/* }}} */
/**
 * DynomiteDB Redis session handler functions
 */
uint32_t
dynomite_hash_murmur(const char *key, size_t length)
{
    /*
...
PHP_REDIS_API void
dynomite_pool_add(redis_pool *pool, RedisSock *redis_sock, int weight, unsigned int tokens, char *prefix, char *auth TSRMLS_DC) {
...
PHP_REDIS_API void
dynomite_pool_free(redis_pool *pool TSRMLS_DC) {
...
PHP_REDIS_API redis_pool_member *
dynomite_pool_get_sock(redis_pool *pool, const char *key TSRMLS_DC) {

    redis_pool_member *dest = NULL;
    if (pool->totalWeight > 1) {
        unsigned int i;
        uint32_t token;
        token = dynomite_hash_murmur(key,strlen(key));
        redis_pool_member *rpm = pool->head;
    ...
    redis_sock_server_open(dest->redis_sock, 0 TSRMLS_CC);
    return dest;
}
    ...
    for (i=0,j=0,path_len=strlen(save_path); i<path_len; i=j+1) {
        ...
        if (i < j) {
            ...
            if (url->query != NULL) {
                ...
                if ((param = zend_hash_str_find(Z_ARRVAL(params), "tokens", sizeof("tokens") - 1)) != NULL) {
                    tokens = zval_get_long(param);
                }
            ...
            if ((url->path == NULL && url->host == NULL) || weight <= 0 || timeout <= 0) {
                ...
                if (prefix) efree(prefix);
                if (auth) efree(auth);
                dynomite_pool_free(pool TSRMLS_CC);
                PS_SET_MOD_DATA(NULL);
                return FAILURE;
            }
            ...
            dynomite_pool_add(pool, redis_sock, weight, tokens, prefix, auth TSRMLS_CC);
...
PS_READ_FUNC(dynomite)
{
    ...
    rpm = dynomite_pool_get_sock(pool, session TSRMLS_CC);
    RedisSock *redis_sock = rpm?rpm->redis_sock:NULL;
    if(!rpm || !redis_sock){
        efree(cmd);
        return FAILURE;
...
PS_WRITE_FUNC(dynomite)
{
    ...
    /* send SET command */
    session = redis_session_key(rpm, key->val, key->len, &session_len);
    cmd_len = redis_cmd_format_static(&cmd, "SETEX", "sds", session, session_len,
                             INI_INT("session.gc_maxlifetime"),
                             val->val, val->len);
    rpm = dynomite_pool_get_sock(pool, session TSRMLS_CC);
    efree(session);
